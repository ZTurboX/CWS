...
parser.add_argument('--mode','-m',type=str,default='train_avg',help='train/train_avg/test')
...
def train(iterations,train_file,beam_size):
    ...
    for t in range(iterations):
        ...
        for line in data:
            ...
            train_seg=' '.join(z)
            seg_data_file = '/home/xzt/CWS/train_seg_data/train-seg-data_ model-' + str(t) + '.txt'
        ...
        model_file=open("/home/xzt/CWS/model_result/model-"+str(t)+"_beam-size-"+str(beam_size)+'.pkl','wb')
...
def train_avg(iterations, train_file, beam_size):
    ...
    avg_model=open("/home/xzt/CWS/model_result/avg-model_beam-size-" + str(beam_size) + '.pkl', 'wb')
...
def test(iterations,test_file,beam_size,mode):
    ...
    for t in range(iterations):
        ...
        model_file=open('/home/xzt/CWS/model_result/model-'+str(t)+'_beam-size-'+str(beam_size)+'.pkl','rb')
        ...
        for line in data:
            z = decoder.beamSearch(line)
            seg_data=' '.join(z)
            seg_data_file='/home/xzt/CWS/test_seg_data/test-seg-data_model-'+str(t)+'_beam-size-'+str(beam_size)+'.txt'
            with open(seg_data_file, 'a') as f:
                f.write(seg_data+'\n')
            count += 1
            if count % 1000 == 0:
                print("segment with model-%d , finish %.2f%%" % (t, (count / data_size) * 100))
        f.close()
        print("segment with model-%d finish"%t)
...
def test_avg(iterations, test_file, beam_size):
    ...
    model_file = open('/home/xzt/CWS/model_result/avg-model_beam-size-' + str(beam_size) + '.pkl','rb')
    feature.load_model(model_file)
    model_file.close()
    for line in data:
        z = decoder.beamSearch(line)
        seg_data = ' '.join(z)
        seg_data_file = '/home/xzt/CWS/test_seg_data/avg-test-seg-data' + '_beam-size-' + str(beam_size) + '.txt'
        with open(seg_data_file, 'a') as f:
            f.write(seg_data + '\n')
        count += 1
        if count % 1000 == 0:
            print("segment with avg-model, finish %.2f%%" %  ((count / data_size) * 100))
    f.close()
    print("segment with avg model finish")
